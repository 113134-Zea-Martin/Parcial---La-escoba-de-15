...
public class EscobaMatchRound {
    ...
    public void dealCards(boolean isPlayerHumanDealer) {
        ...
        // TODO: Implementar lógica para repartir cartas intercaladamente una para cada jugador hasta repartir 3 a cada uno.DONE
        List<Card> appHand = new ArrayList<>();
        List<Card> humanHand = new ArrayList<>();
        if (isPlayerHumanDealer) {
            for (int i = 0; i < 3; i++) {
                appHand.add(deck.takeCard());
                humanHand.add(deck.takeCard());
            }
        } else {
            for (int i = 0; i < 3; i++) {
                humanHand.add(deck.takeCard());
        ...
        roundPlayerApp.setHandCards(appHand);
        roundPlayerHuman.setHandCards(humanHand);
    ...
    public void playRound(boolean isPlayerDealer) {
        ...
        do {
            ...
            if (!isRoundFinish()) {
                ...
                LetterByLetterPrinter.println("Cartas en la Deck: " + deck.getCards().size());
            }
        } while (!isRoundFinish());
        calculateRoundScore();
    }
    ...
    private void solveSevenWinner() {
        ...
        } else if (humanSevens < appSevens) {
            LetterByLetterPrinter.println("La app gana 1 punto por tener la mayor cantidad de sietes.");
            roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            if (appSevens == 4) {
                LetterByLetterPrinter.println("La app gana 1 punto extra por tener todos los sietes.");
                roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            }
        } else {
            LetterByLetterPrinter.println("Ambos jugadores tienen 2 sietes, no se llevan puntos.");
        }
        // TODO: Implementar lógica para resolver quien gana el punto por cantidad de siete.DONE
    ...
    private Boolean sevenOroCheck(List<Card> cards) {
        ...
        // TODO: Implementar lógica para verificar si existe el siete de oro en la lista de cartas.DONE
        return false;
    }
